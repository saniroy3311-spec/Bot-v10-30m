...
from config import (
    ...
    MIN_ATR_FOR_ENTRY,     # FIX-PROFIT-FILTER: skip low-volatility signals
    RANGE_SIGNALS_ENABLED, # FIX-PROFIT-FILTER: allow disabling Range signals
)
...
def evaluate(snap: IndicatorSnapshot, has_position: bool = False) -> Signal:
    ...
    # FIX-PROFIT-FILTER: skip signals when ATR is too low for the expected
    # move to clear round-trip commission. Applies to ALL signal types
    # (trend + range) before any other condition is checked.
    if snap.atr < MIN_ATR_FOR_ENTRY:
        return Signal(SignalType.NONE, False, False, "NONE")

    f  = snap.filters_ok
    tr = snap.trend_regime
    rr = snap.range_regime and RANGE_SIGNALS_ENABLED  # FIX-PROFIT-FILTER
