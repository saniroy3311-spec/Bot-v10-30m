...
# FIX-DRY-RUN: use REAL API key + REAL live market data/prices, but block
# every order-placing/closing/bracket call from reaching the real account.
# Entries/exits are simulated using live ticker price instead.
# Use this when testnet isn't available/working but you still want to
# paper-trade against real market conditions with zero real-money risk.
DRY_RUN = os.environ.get("DRY_RUN", "false").lower() == "true"
SYMBOL    = os.environ.get("SYMBOL",    "BTC/USD:USD")

# FIX-INSTANCE-NAME: lets you run multiple copies of this bot on the same VPS
# (e.g. one on 15m, one on 30m candles) and tell them apart in Telegram
# messages, logs, and the dashboard title. Set a different value per .env.
BOT_INSTANCE_NAME = os.environ.get("BOT_INSTANCE_NAME", "Shiva Sniper Bot v10")
...
# FIX-PROFIT-FILTER: minimum ATR required before ANY signal is allowed to fire.
# Purpose: skip low-volatility setups where the expected move can't clear
# round-trip commission (COMMISSION_PCT) at current position size.
# Tune via .env — raise it to trade less/higher-quality, lower to trade more.
# Set to 0 to disable (restores old always-fire behavior).
MIN_ATR_FOR_ENTRY = float(os.environ.get("MIN_ATR_FOR_ENTRY", "250"))

# FIX-PROFIT-FILTER: range trades (Range Long/Short) historically had the
# worst hit rate in live trading. This flag lets you disable them entirely
# via .env without touching code, while trend trades keep running normally.
RANGE_SIGNALS_ENABLED = os.environ.get("RANGE_SIGNALS_ENABLED", "true").lower() == "true"
BREAKOUT_BUFFER_PTS = 0
